...
class Zero(sabre.Abr):
   ...
   # A_DIM = len(VIDEO_BIT_RATE)
    ...
    def get_quality_delay(self, segment_index):
        ...
        if segment_index < 0:
            return
        manifest_len = (len(sabre.manifest.segments) + 1) * sabre.manifest.segment_time
        ...
        state[6, -1] = min(sabre.rebuffer_time / (sabre.total_play_time + EPS), 1.0)
        state[7, -1] = min(sabre.total_bitrate_change / \
            (sabre.played_bitrate + EPS), 1.0)
        state[8, -1] = (len(sabre.manifest.segments) - segment_index) / len(sabre.manifest.segments)
        state[9, -1] = sabre.get_buffer_level() / (manifest_len)

        for p in range(10):
            if state[p,-1] > 1.0:
                self.global_throughput = max(self.global_throughput, throughput)
